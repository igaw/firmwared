...
static void setup_firmware_files(const struct config_data *cfg) {
        ...
        asprintf(&fullpath, "%s/%s", cfg->path, cfg->filename);
...
static void buf_freep(char **buf) {
        if (*buf)
                free(*buf);
}
...
static bool check_content(const char *filename, const char *content) {
        ...
        _cleanup_(buf_freep) char *buf = NULL;
        ssize_t buf_len;
        ssize_t len;
...
{
        if (WIFEXITED(status))
                tester_warn("process %d exited with status %d",
                        pid, WEXITSTATUS(status));
        else if (WIFSIGNALED(status))
                tester_warn("process %d terminated with signal %d",
                        pid, WTERMSIG(status));
        else if (WIFSTOPPED(status))
                tester_warn("rocess %d stopped with signal %d",
                        pid, WSTOPSIG(status));
        else if (WIFCONTINUED(status))
                tester_warn("process %d continued", pid);
}
